...
## BEGIN MD5 implementation
# MD5 used to compute type signatures

MD5_dlen: con 16;
MD5_block_size: con 64;
MD5_max_32 : con int 16rffffffff;

MD5_shift_map := array [] of {
    7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22,
    5,  9, 14, 20, 5,  9, 14, 20, 5,  9, 14, 20, 5,  9, 14, 20,
    4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23,
    6, 10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21};

MD5_sine_table := array [] of {
...
MD5_acc(block: array of byte, result: array of int)
{
    ...
    for (i := 0; i < MD5_block_size; i++)
    {
        ...
        {
            0 to 15 =>
                f = (b & c) | ((MD5_max_32 ^ b) & d);
                g = i;
            16 to 31 =>
                f = (b & d) | (c & (MD5_max_32 ^ d));
                g = ((5 * i) + 1) % 16;
            32 to 47 =>
                f = b ^ (c ^ d);
                g = ((3 * i) + 5) % 16;
        ...
        b = b + MD5_leftrotate((a + f + MD5_sine_table[i] + v), MD5_shift_map[i]);
        a = tmp;
    }
...
MD5_pad_block(block: array of byte, curr_block_size: int, msg_size_bytes: big): array of byte
{
    if (curr_block_size > len(block) || len(block) != MD5_block_size) {
    ...
    pad_count := MD5_block_size - curr_block_size;

    # Check if there is room for the last bit and message size
    if (pad_count < 9) {
        aux_block = array [MD5_block_size] of byte;
    }
...
MD5_typesig(buf: array of byte, n: int, digest: array of byte)
{
    checksum := array [] of {
        int 16r67452301,
        int 16refcdab89,
    ...
    block := array [MD5_block_size] of byte;
    c := MD5_block_size;
	if (n < c) {
		c = n;
	}
	...
	n -= c;
    msg_size_bytes := 0;
	block[0:] = buf[msg_size_bytes:c];
    while (c == MD5_block_size) {
        msg_size_bytes += c;
		...
		n -= c;
		block[0:] = buf[msg_size_bytes:msg_size_bytes + c];
    }

    msg_size_bytes += c;

    # Pad the last block
    aux_block := MD5_pad_block(block, c, big msg_size_bytes);
    MD5_acc(block, checksum);
    if (aux_block != nil) {
...
sign(d: ref Decl): int
{
	...
	md5sig := array[MD5_dlen] of {* => byte 0};
	MD5_typesig(sig, sigend, md5sig);

	for(i := 0; i < MD5_dlen; i += 4)
